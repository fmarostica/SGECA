...
namespace SGECA.ColaDeImpresion.builders
{
    public class LibroDeIva
    {
        ...
        public void quitarDocumento(string idDocumento)
        {
            ...
            MySqlCommand cmd = new MySqlCommand("UPDATE libroivaventas SET lvt_estadoimpresion = 1, " +
                                                "lvt_fechaimpresion = NOW() " +
                                                "WHERE lvt_id=@lvt_id", cone);
            cmd.Parameters.AddWithValue("@lvt_id", id);
            try
            {
                cone.Open();
                cmd.ExecuteNonQuery();

                LogManager.Mensaje m = new LogManager.Mensaje("SGECA.ColaDeImpresion.builders.LibroDeIva",
                                           "quitarDocumento",
                                           0,
                                           "El libro de IVA se quito correctamente de la cola de impresión.",
                                           "",
                                           "",
                                           false,
            ...
            catch (Exception Exception)
            {
                ...
                LogManager.Mensaje m = new LogManager.Mensaje("SGECA.ColaDeImpresion.builders.LibroDeIva",
                                            "quitarDocumento",
                                            0,
                                            "Error al intentar quitar el libro de IVA de la cola de impresión.",
                                            Exception.Message,
                                            "",
                                            true,
        ...
        public List<object> obtener()
        {
            ...
            MySqlConnection cone = new MySqlConnection(ConfigurationManager.ConnectionStrings["ce"].ConnectionString);
            MySqlCommand cmd = new MySqlCommand("select * from libroventa where lvt_estado is null", cone);
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    var item = new
                    {
                        ...
                        copias = short.Parse(dr["gim_copias"].ToString()),
        ...
        public void eliminar(string idDocumento)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("UPDATE libroivaventas SET lvt_estadoimpresion = 2, " +
                                                    "lvt_fechaimpresion = NOW() " +
                                                    "WHERE lvt_id=@lvt_id", cone);
                cmd.Parameters.AddWithValue("@lvt_id", id);
                cone.Open();
