...
namespace SGECA.DAL
{
    public class Cliente : LogManager.ISubject, LogManager.IObserver
    {
        ...
        public void obtener(int id)
        {
            ...
            MySqlCommand comando = new MySqlCommand(
                "SELECT * " +
                "FROM vw_cliente WHERE cli_id = @cli_id"
                , conexion);
        ...
        private void cargarDatos(Cliente objeto, MySqlDataReader dr)
        {
            ...
            objeto.RazonSocial = dr["cli_razonsocial"].ToString();

            //int aux = 0;
            //int.TryParse(dr["cli_CUIT"].ToString(), out aux);
            //objeto.cli_cuit = aux;
            objeto.CUIT = dr["cli_CUIT"].ToString();


            if (dr["cli_ingresosbrutos"] != null && dr["cli_ingresosbrutos"].ToString().Length > 0)
                objeto.IngresosBrutos = dr["cli_ingresosbrutos"].ToString();

            objeto.TipoResponsableCodigo = dr["tre_codigo"].ToString();


            objeto.Codigo = dr["cli_codigo"].ToString();
            ...
            objeto.Domicilio = dr["dir_Calle"].ToString();
            if (dr["dir_Altura"] != null)
                objeto.Domicilio += " " + dr["dir_Altura"].ToString();
            if (dr["dir_Piso"] != null && dr["dir_Piso"].ToString().Length > 0)
                objeto.Domicilio += " - Piso: " + dr["dir_Piso"].ToString();

            objeto.Localidad += dr["loc_nombre"].ToString();
