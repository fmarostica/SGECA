...
namespace SGECA.DAL
{
    public class AlicuotaIva : LogManager.ISubject, LogManager.IObserver, IAlicuotaIva
    {
        #region declaración de Propiedades
        public int Codigo { get; set; }
        public string Nombre { get; set; }
        public decimal ali_porcentaje { get; set; }
        ...
        //la programación
        private static string dbCampoId = "ali_id";
        private static string dbCampoNombre = "ali_descripcion";
        private static string dbCampoValor = "ali_porcentaje";
        private static string dbTabla = "alicuota";
        ...
        public void obtener(decimal alicuota)
        {
            limpiaDatos();
            ...
            MySqlCommand comando = new MySqlCommand(
                "SELECT * " +
                "FROM  " + dbTabla + " " +
                "WHERE  " + dbCampoValor + " = @" + dbCampoValor
                , conexion);

            comando.Parameters.AddWithValue(dbCampoValor, alicuota);
            comando.Transaction = Database.obtenerTransaccion();
        ...
        private static void cargarDatos(AlicuotaIva objeto, MySqlDataReader dr)
        {
            int id = 0;
            ...
            decimal aux = 0;
            decimal.TryParse(dr[dbCampoValor].ToString(),out aux);
            objeto.ali_porcentaje = aux;
